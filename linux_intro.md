...
4. Verify your username by typing `whoami`. This command is useful in computer labs if you come across an unoccupied terminal that appears to be in use, and you would like to know who is logged in. Also, it gives you an idea of the current user load on the machine.
...
11. Brief text editor review. Two editors that are commonly used in the Unix environment are **vi** and **emacs**. **emacs** is more sophisticated, but requires more effort to master. If you want to try it out, type `emacs` at the Unix prompt, and follow the instructions to work through the tutorial. For right now, we will use **vi**. (Note: there are many useful online quick references for vi. See, for example: [http://it.unm.edu/quickrefs/vi.html](http://it.unm.edu/quickrefs/vi.html)].  
    ...
    **`k`** to move up a line  
    **`D`** to delete from cursor location to the end of the line  
    **`dd`** to delete entire line  
    **`:`** to get into editor mode. From here, you can type **`w`** (followed by the Enter key) to write your file (this will overwrite whatever file you said you were editing when you invoked vi), **`:wq`** to write and quit vi; and **`:q`** to exit without writing over your file. Another very useful editor-mode command is **`u`** (typed in normal mode, without the colon); this undoes your last command in case you made a mess of your editing.  
    Type **`<number>G`** in normal mode to go to line number **`<number>`**. **`$`** refers to the last line in your file, so **`$G`** will take you to the last line in your file.  
    Also in normal mode, type **`<Ctrl>-G`** (this is the Ctrl key on your keyboard, held down at the same time as the G key) to find out what line you are at.

12. Now that you are an editing expert, create a new file called **junk.txt** containing the single line "Hello World." You may use **vi** or the Unix editor of your choice to do this.
