...
```bash
$ conda activate py-3.13

(py-3.13)$ pip install psutil==5.3.0
...
# packages in environment at /users/yourusername/.conda/envs/py-3.13:
#
# Name                    Version                   Build
ca-certificates           2018.03.07                    0
certifi                   2018.8.24               py313_1
...
pip                       10.0.1                  py313_0
**psutil                  5.3.0                     <pip>
python                    3.13.5               h1571d57_0
readline                  7.0                  h7b6447c_5
setuptools                40.2.0                  py313_0
sqlite                    3.24.0               h84994c4_0
tk                        8.6.8                hbc83047_0
wheel                     0.31.1                  py313_0
...
Sometimes the default repositories, or channels, for Conda do not have the package you are looking for, but that does not mean it is necessarily unavailable entirely. Say you are working with some Illumina sequence data and need the Burrows-Wheeler Aligner (bwa) in your pipeline, so you activate your bioinformatics environment and type `conda install bwa`, which prints the following:
...
We can search other channels that may have the package we are interested in using the `-c` flag. For example, Bioconda is a large repository that hosts several thousand bioinformatics packages. We can search for the bwa package by specifying that channel:
...
We can then install the bwa package using `conda install -c bioconda bwa` and continue with our analyses. You can permanently add channels by appending your `.condarc` file either directly in a text editor, or with `conda` using the `config` command:
...
For more information on managing channels and installing with pip, please refer to the Conda documentation at this [link](https://conda.io/docs/user-guide/tasks/manage-channels.html).
